...
void engine::driveraidsOn()
{
	driveraids = true;
}

void engine::driveraidsOff()
{
	driveraids = false;
}
...
//#define Z2DCALC(qz, zz, ww)  qz = (( ( 1.0f - (ww = 3.0f * RECIPROCAL( zz ))) ) * zNormalizer ); if( qz < 0.0f ) qz = 0.0f;

#define Z2DCALC(qz, zz, ww)  qz = (( ( 1.0f - (ww = (( zz > 500.0f) ? (zz/500.0f): 1.0f) * RECIPROCAL( zz ))) ) * zNormalizer ); if( qz < 0.0f ) qz = 0.0f;
...
__inline void engine::project( vector2P * v2,  vector3 * v, float l, float sl )
{
...
__inline void engine::project( vector2P * v2,  float vx, float vy, float vz, float l, float sl )
{
...
__inline void engine::zLimitedproject( vector2P * v2,  vector3 * v, float l, float sl )
{
...
__inline void engine::zLimitedproject( vector2P * v2,  float vx, float vy, float vz, float l, float sl )
{
...
void engine::setCurrentCamera( camera * c )
{
	...
	zNormalizer1 *= 8191.0f;//2730.0f ;//* 0.25f;
#else
...
return;
	...
	float t, r, xd, yd, ud, vd, ld, specularld;
	...
	vd =   b->nv.v - a->nv.v;
	ld =   b->v->l - a->v->l;
	specularld =   b->v->specularl - a->v->specularl;
	...
	project( outNextVector2Pptr, avg->x + (float)(t * xd), avg->y + (float)(t * yd),
						splitz0, a->v->l + (float)(t * ld),  
								 a->v->specularl + (float)(t * specularld)
			);
	...
	outNextNonGeoptr->u = a->nv.u + (float)(t * ud);
	outNextNonGeoptr->v = a->nv.v + (float)(t * vd);
	project( outNextVector2Pptr, avg->x + (float)(t * xd), avg->y + (float)(t * yd), splitz1,
				a->v->l + (float)(t * ld),
				a->v->specularl + (float)(t * specularld)
			);
...
//	outNextVector2Pptr1->w =  av->w + t1 * wd;
    ...
    Z2DCALC(outNextVector2Pptr1->z, outNextVector2Pptr1->v3.z, outNextVector2Pptr1->w);	
				...
				if( checkRboundboxVisibility( &myObject->transformedBoundingBox  ) )
				{
					...
					for( t = myObject->modelData->meshData.tcount; t--; )
					{
						...
						if(shadowTri)
						{
							...
							potentiallyVisible = true; //(  dp  <= SEPSILON) ;
						};
				 if( potentiallyVisible)
				 {
						if( pvPtr->tcnt != globalpctr )		// not yet transformed 
...
pvPtr->transformed.z -= zoomz;
	...
	if( pvPtr->specularl < 0 ) pvPtr->specularl = 0;
...
qvPtr->transformed.z -= zoomz;
	...
	if( qvPtr->specularl < 0 ) qvPtr->specularl = 0;
...
rvPtr->transformed.z -= zoomz;
	...
	if( rvPtr->specularl < 0 ) rvPtr->specularl = 0;
...
_inline ulong engine::formHwareKeyAlpha( ulong k,  float f0, float f1, float f2 )
{
	slong l;
float f;
...
#define OCCLUDER
		  ...
		  sort( (count = llist - currentOutListPtr->triangleList), 
			...
			//   (i) Add all triangles to a list sorted by :
			//			alpha,  z (ascending)
			//  (ii) Sort all triangles
			// (iii) Occlude
			//  (iv) reorder as before.
...
/*
			float minz = tlist->p.v->v3.z;
			if( minz < tlist->q.v->v3.z )
				minz = tlist->q.v->v3.z;
			if( minz < tlist->r.v->v3.z )
				minz = tlist->r.v->v3.z;
*/		
	
float py = tlist->p.v->y;  if(py < 0) py = 0; else if (py > displayH) py = displayH;
float qy = tlist->q.v->y;  if(qy < 0) qy = 0; else if (qy > displayH) qy = displayH;
float ry = tlist->r.v->y;  if(ry < 0) ry = 0; else if (ry > displayH) ry = displayH;


float ypart =  (py + qy + ry) * 0.3333f ;
			  
float minz = (tlist->p.v->v3.z + tlist->q.v->v3.z + tlist->r.v->v3.z ) * 0.3333f * 65536
			  + ypart;


			FASTFTOL(  (long *)(&thiskey),  minz );//* 65536.0f );
			//if( tlist->mtl->bm.flags & MAT_ALPHA ) 
			//	thiskey |= 0x080000000;
		  ...
		  sort( (count = llist - currentOutListPtr->triangleList), 
			...
			//
			// Now we should have ALL the triangles z-ordered front to back (ascending) regardless
			// of alpha or anything else. We therefore occlude by setting the 'idx' field of an occluded
			// triangle to '0x0FFFFFFFF'
			//


			//
			// IMPORTANT NOTE - for occlusion we assume that the triangles are completely on the 2D
			// screen - this will already have happened and we dont need to split them!!!
			//
		...
		long * occPtr = occlusionColumn;
	
			// clear occlusion columns 
		...
		_asm{
			...
			mov edx, ll
			mov eax, occPtr
			shr edx, 1				// displayW is ALWAYS even!!!
			mov ebx, 9999
			dec edx
			L00:
				mov dword ptr [eax + edx*8], ebx
				mov dword ptr [eax + edx*8 + 4], ebx
				dec edx
			jns L00
		};
		...
		for( ii = count; ii--; )
		{
			t = ((outTriangle *)(sortlistPtr->idx));

			// Test this triangle against the occlusion columns

		
			long minx, maxx, minx_y, maxx_y, other_y, other_x;
			float fminx, fmaxx, fminx_y, fmaxx_y, fother_y, fother_x;
			...
			{	
				...
				miny = t->p.v->y;
				if( miny > t->q.v->y ) miny = t->q.v->y;
				if( miny > t->r.v->y ) miny = t->r.v->y;

				long ylo = (long) miny;

				occPtr = &occlusionColumn[minx];
				occluded = true;
			
				maxx = (long)fmaxx;
				minx = (long)fminx;
				other_x = (long)fother_x;
				maxx_y = (long)fmaxx_y;
				minx_y = (long)fminx_y;
				other_y = (long)fother_y;
				...
				if( occluded )
				{
					sortlistPtr->idx = 0x0FFFFFFFF;	
boris2->occluded++;					
				} else
				{
					//if( ((t->mtl->bm.flags & MAT_ALPHA) == 0)  &&
					if ( (t->mtl->bm.flags & MAT_OCCLUDE) && ((t->mtl->bm.flags & MAT_ALPHA) == 0) )// Triangle is NOT alpha and is NOT occluded
					{

							// ... update the occlusion columns for this triangle!!
						...
						occPtr = &occlusionColumn[other_x];
...
//};
			...
			//
			// From the list which has been sorted for occlusion, redefine the keys and re-order
			// for rendering.
			//
		  ...
		  for( ii = count; ii--; )
		  {
			if(sortlistPtr->idx != 0x0FFFFFFFF )
			{
			 t = ((outTriangle *)(sortlistPtr->idx));

			 if( (thiskey = t->mtl->sortingKey) & 0x080000000 )	// alpha texture
					thiskey = formHwareKeyAlpha(thiskey, t->p.v->v3.z,t->q.v->v3.z,t->r.v->v3.z );
			 else
					thiskey = formHwareKeyNonAlpha(thiskey, t->p.v->v3.z,t->q.v->v3.z,t->r.v->v3.z );
...
//
// 0.07  ...  replace values in sprite description file instead!!!
//
		boris2->currentcamera->transformWorldVector(&tempcoord, &( thisFXentry->worldPosition +
							thisFrame->offset * 0.07f)	);
